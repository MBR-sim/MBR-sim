    ...
    with open(file, "r") as csvfile:
        ...
        for row in csvreader:
            if ("".join(row))[0] == "#":
                ...
                last_elem = row[-1]
                if isnumber(last_elem) and int(last_elem) > 1:
                    rep = int(last_elem)
                    for i in range(rep):
                        csvwriter.writerow(row)
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--file", help="the path to the csv file to be unrolled", type=str, default="src/MBR_sim/workloads/csv/bert-large.csv")
